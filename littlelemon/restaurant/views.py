...
from .forms import BookingForm, RegestrationForm
...
# All User Registrations Page

def all_registrations(request):
    # Fetch all registrations from the Registering model
    registrations = Registering.objects.all()
    
    # Serialize the registration data to JSON
    registrations_json = serializers.serialize('json', registrations)
    
    # Return the JSON response or render it in the template
    return render(request, 'registrations.html', {"registrations": registrations_json})
...
def registrations(request):
    if request.method == 'POST':
        try:
            ...
            if Registering.objects.filter(username=username).exists():
                return JsonResponse({'error': 'Username already exists'}, status=400)

            if Registering.objects.filter(email=email).exists():
                return JsonResponse({'error': 'Email already exists'}, status=400)

            # Create the user
             # Hash the password before saving
            hashed_password = make_password(password)
            new_user = Registering(
                username=username,
                email=email,
